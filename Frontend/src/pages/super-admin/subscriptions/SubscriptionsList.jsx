...
export default function SubscriptionsList() {
  ...
  const handleAction = (id, actionType) => {
    ...
    if (actionType === "extend") {
      ...
      superAdminService.updateBusiness(id, { subscriptionExpiry: d.toISOString().slice(0, 10) });
      alert(`Extended subscription for ${bus.name} by 30 days.`);
    } else if (actionType === "pause") {
      superAdminService.updateBusiness(id, { status: "Suspended" });
      alert(`Paused subscription for ${bus.name}.`);
    } else if (actionType === "resume") {
      superAdminService.updateBusiness(id, { status: "Active" });
