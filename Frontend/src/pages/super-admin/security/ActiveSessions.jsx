...
import { Laptop, Trash2, ShieldAlert, CheckCircle, Clock } from "lucide-react";

export default function ActiveSessions() {
  const [sessions, setSessions] = useState(() => superAdminService.getSessions());

  const handleTerminate = (id) => {
    const updated = superAdminService.terminateSession(id);
    setSessions(updated);
  };

  const handleTerminateAll = () => {
    if (confirm("Terminate all other Super Admin sessions except this active browser window?")) {
      const updated = superAdminService.terminateAllOtherSessions();
      setSessions(updated);
    }
  };
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-gray-900">{sess.device}</span>
                </div>
                {sess.current && (
              ...
              <div className="space-y-1.5 text-xs text-gray-600">
                <div>Admin Operator: <strong className="text-gray-900">{sess.admin}</strong></div>
                <div>Browser: <span className="text-gray-700">{sess.browser}</span></div>
                <div>IP Address: <span className="font-mono text-gray-800">{sess.ip}</span></div>
                <div>Session Started: <span className="font-mono text-gray-500">{sess.loginTime}</span></div>
                <div>Last Telemetry: <span className="text-emerald-600 font-semibold">{sess.lastActivity}</span></div>
              </div>
            </div>
